...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagcolor_controls)
    {
        info->event[info->num] = EVENT_color_controls;
        info->num++;
        dataPoints->valuecolor_controls = gizStandardDecompressionValue(color_controls_BYTEOFFSET,color_controls_BITOFFSET,color_controls_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuecolor_controls != cur->valuecolor_controls)
    {
        GIZWITS_LOG("valuecolor_controls Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valuecolor_controls >= color_controls_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuecolor_controls Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(color_controls_BYTEOFFSET,color_controls_BITOFFSET,color_controls_LEN,(uint8_t *)devStatusPtr,dataPoints->valuecolor_controls);  
    }
